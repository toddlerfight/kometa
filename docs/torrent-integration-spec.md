...
- **Komga:** library id `<your-komga-library-id>`, root `/comics`. Scan trigger via
  `_komga_scan()` (acquisition.py:90).
- **Container-DNS rule:** services on the NAS address each other by LAN IP
  `$NAS_HOST` or `127.0.0.1`, never the Tailscale MagicDNS name (containers
...
- Kometa container → qBit: reach the WebUI via the host LAN IP
  `http://$NAS_HOST:8090` (container-to-host), per the container-DNS rule.
